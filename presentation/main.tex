...
Give the model a \emph{trusted} target for each forget sample, then
...
% ─────────────────────────────────────────────────────────────────────────────
\begin{frame}{Seeing the divergence in activation space}

\centering
\includegraphics[width=0.96\textwidth]{08_effect_before_after.pdf}

\vspace{0.5mm}
\begin{columns}[T]
\begin{column}{0.5\textwidth}
\footnotesize
\textbf{Left --- \emph{where} they sit.}
One axis (LDA) is fit on the \emph{teacher} to best tell forget from retain at block~5;
every sample is projected onto it. Filled $=$ before, outline $=$ after.
The \textcolor{cForget}{forget} cloud slides $\sim$3 units off its baseline location;
\textcolor{cRetain}{retain} does not move.
\end{column}
\begin{column}{0.5\textwidth}
\footnotesize
\textbf{Right --- \emph{how far} each moved.}
Per-sample $\norm{\mathbf{a}_{\text{after}}-\mathbf{a}_{\text{before}}}_2$.
\textcolor{cRetain}{Retain} barely shifts (mean 0.8); \textcolor{cForget}{forget}
travels $3\times$ further (mean 2.4), the two hardly overlap.
\end{column}
\end{columns}
...
\centering
\small
Relabelling only remaps the output; here the \textbf{internal representation itself moves and only for forget}.
...
The two families make \emph{complementary} errors:
